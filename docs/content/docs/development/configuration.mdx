...
| `DB_MAX_CONNS` | Maximum pooled Postgres connections for one Go process (backend, consumer). The ceiling that matters is the database's `max_connections`, and every replica, consumer and `warmblyctl` run takes its own share of it. At boot each process reads that ceiling and lowers its own pool to a quarter of the connections the server leaves unreserved, so a database too small for the fleet is survived rather than exhausted; the value here is an upper bound, never a reservation, and the clamp only ever lowers it. A `pool_max_conns` in the DSN is honoured when this is unset | `25`, clamped to the server's capacity | yes |
