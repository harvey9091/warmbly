...
The ordinary **Forgot password** flow still works, if mail delivers. It is capped at **2 requests per address per 4 hours**, so repeated attempts during an outage will lock you out of the flow for a while. The emailed link is good for **1 hour**.

The address is matched case-insensitively and surrounding whitespace is ignored, so `You@Example.com` finds the account created as `you@example.com`. An address with no account still answers `200` on purpose, so the form cannot be used to find out who has an account here; the backend logs the attempt, so an operator can tell a typo apart from a mail failure.
