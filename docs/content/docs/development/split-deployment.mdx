...
`certbot-deploy-hook.sh` belongs in `/etc/letsencrypt/renewal-hooks/deploy/`. Neither service re-reads its certificate, so a renewal without it leaves both serving an expired one.
