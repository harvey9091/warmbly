...
It installs Docker and certbot if they are missing, creates the group that may read the private key, generates the two credentials, obtains the certificate, installs the renewal hook, starts both services, waits for them to report healthy, proves a real client can connect over TLS with each credential and that a wrong one is refused, and prints the two lines your control plane needs.
...
Two flags worth knowing: `--verify` re-runs the connection checks against a box that is already up, and `--print-env` prints the control plane lines again.
