...
| Import column mapping | The key alone | For an import or Google Sheets preview with columns no header matched: every column header, the kind of value each column holds (email addresses, dates, short text and so on, worked out on the instance), and the workspace's custom field names. Never a cell value, and nothing at all when the header row holds an address, a date or a number, since that row is a contact rather than headers | Nothing is stored; the answer only fills in the suggested mapping |
