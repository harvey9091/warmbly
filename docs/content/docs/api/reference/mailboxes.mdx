...
Nothing is removed unless every step that cannot be repaired afterwards succeeds first: a mailbox enrolled in [Warmbly Cloud](/guides/warmbly-cloud/) has its enrollment revoked, which is what takes its stored credentials out of the pool, and the machine syncing it is told to drop it. A failure at either point leaves the mailbox exactly as it was and is safe to retry.
