...
Returns the number of unread conversations in the Inbox folder, optionally scoped to one mailbox. Snoozed conversations and mail in other folders (sent, drafts, archive, spam, trash) are not counted, so the number matches what the Inbox view lists as unread. Backs the inbox badge. Auth: **Scope** `READ_UNIBOX` · **Org permission** `access_unibox`.
