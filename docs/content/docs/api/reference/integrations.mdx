...
Lists this org's saved sync sources, optionally filtered to a campaign or a segment.
...
| `segment_id` | query | uuid | Optional. Restricts to sources that pin their rows into that segment. Invalid ids return `400`. Combines with `campaign_id`. |
