...
The three `partner_*_7d` counters are the distinct warmup partners the mailbox reached over the last seven days: mailboxes, their domains, and the workspaces behind them. `partner_organizations_7d` of `1` over a full week means the mailbox is only warming inside one workspace, which is expected on a self-hosted instance that is not linked to Warmbly Cloud and worth investigating anywhere else.
