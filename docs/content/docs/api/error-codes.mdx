...
| `invalid_sort_by` | `POST /contacts/search` was given a `sort_by` of the form `custom:<key>` whose key could never be a custom-field name (letters, numbers, underscores, spaces or dashes) |
| `unknown_view` | `/me/views/:view` was given a view name other than `contacts` or `campaign_leads` (answered `404`) |
| `invalid_column`, `duplicate_column`, `too_many_columns`, `invalid_sort` | `PUT /me/views/:view` was given a column id the dashboard cannot render, the same column twice, more than 64 columns, or a sort that names neither a built-in column nor a well-formed `custom:<key>` |
