...
A `400` whose `code` is `mailbox_validation_timeout` comes from the SMTP and IMAP connect endpoints. Warmbly proves credentials by opening a real connection to the mail server before it stores anything, and the server did not answer inside that window. Nothing was saved and no mailbox was created.

```json
{
  "error": "Bad Request",
  "message": "Warmbly could not get an answer from this mail server in time. Nothing was saved. Check the host and port, then try again.",
...
**How to fix:**
- Check the host and port. A port that nothing listens on takes the full timeout rather than refusing straight away
...
{
  "error": "Bad Request",
  "message": "SMTP (smtp.gmail.com:465) refused the sign-in: 535 5.7.8 Username and Password not accepted. IMAP (imap.gmail.com:993) refused the sign-in: imap: NO [AUTHENTICATIONFAILED] Invalid credentials (Failure). Nothing was saved. Google accepts only a 16-letter app password here, created at myaccount.google.com/apppasswords on the same Google account as the address, with the username being the full address. The account password does not work, and on Google Workspace the administrator can switch IMAP or app passwords off.",
...
A `400` whose `code` is `mailbox_unreachable` comes from the same endpoints. The worker could not open a connection to the server at all: the name did not resolve, the port refused, or a firewall dropped the packets. The message says which server and what the dial reported. Nothing was saved.

```json
{
  "error": "Bad Request",
  "message": "SMTP (smtp.example.com:465) could not be reached from the worker: dial tcp 203.0.113.9:465: connect: connection refused. Nothing was saved.",
...
A `400` whose `code` is `mailbox_server_declined` comes from the same endpoints. The server was reached and spoke, but the sign-in could not be completed for a reason other than the credentials: it asked for a retry later (a `4xx` reply, often a login rate limit), it offers no sign-in method Warmbly implements, or the conversation broke. The message carries the server's reply. Nothing was saved.
...
- For a server that offers only mechanisms Warmbly does not implement (NTLM, GSSAPI), use the provider's documented SMTP submission host, or an app password
