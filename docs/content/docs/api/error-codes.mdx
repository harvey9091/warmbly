...
{
  "error": "Bad Request",
  "message": "SMTP (smtp.gmail.com:465) did not answer in time. IMAP (imap.gmail.com:993) signed in. Nothing was saved. Check the host and port, then try again. Some networks block outbound port 465: if the server also offers 587, connect with 587 and STARTTLS.",
...
- Outbound 465 is blocked by default on many hosting providers while 587 stays open. If the server offers both, use 587 with `starttls`
