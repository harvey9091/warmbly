...
- **Triage** classifies every submission as a **buyer** (a prospective customer or someone asking about what you offer), a **vendor** (someone pitching you), a **job seeker**, **other**, or **junk** (spam, gibberish, a test entry, or an unsolicited pitch). The verdict shows as a chip on the submissions list and can be filtered on. Junk is stored and flagged but creates no contact and joins no campaign, so a bot cannot put itself in your outreach. Everything else is handled exactly as before. Triage is off for each form until you switch it on, and the toggle appears once the operator has configured a `TYPESAFE_API_KEY` for the instance. The answers the visitor typed (and nothing about their connection) are sent to TypeSafe for the judgment; a slow or failed call leaves the submission untriaged rather than delaying the visitor.
