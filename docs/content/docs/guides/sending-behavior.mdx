...
Every time is local to the mailbox's timezone: its own, set on the **Settings** tab of its drawer or under **Settings > Profile > Timezones**, or the workspace timezone (same page) when it has none.

A new mailbox has no timezone of its own, and that is a valid state rather than a gap: its workday and warmup hours are read in the workspace timezone (UTC when that is not set either), and on campaigns only the campaign's own sending window applies. Set one when the mailbox belongs to somebody in a particular place, and both calendars then have to agree before a send goes out.
