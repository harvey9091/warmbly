...
| View | Shows |
| --- | --- |
| Hot leads | Replies that agreed, proposed a time, asked for a call or asked about pricing |
...
| Declined | Not interested, wrong person, or asked to be removed |
| Automated | Bounces, autoresponders and platform notices |
...
Nothing needs refreshing. New mail appears in the list, updates the open thread, and raises the unread count on its own. The count on **Unibox** in the navigation (and in the browser tab) is the number of unread conversations in **Inbox**, the view it opens, so it never points at mail that view does not list: sent mail, drafts, archived and snoozed conversations are left out. Read-state changes, replies, scheduled sends, snoozes, and label changes all refresh the affected views, keeping rail counts, the list, and the open thread consistent.
