...
<Callout type="info" title="Pricing">
  A free Warmbly Cloud workspace holds up to 10 mailboxes, connected directly or through a linked instance, and warms them at no cost. Unlimited mailboxes cost $15 a month per workspace, or $144 a year. Nothing else about self-hosting changes.
</Callout>
...
Unenrolling an SMTP/IMAP mailbox, disconnecting the instance, or deleting the mailbox altogether deletes its credential on the cloud immediately. Deleting an enrolled mailbox revokes its enrollment before anything local is removed, so the pool cannot be left holding a password for a mailbox your instance no longer has; if the cloud cannot be reached at that moment the delete is refused rather than done halfway, and retrying it once the cloud is reachable completes it. A mailbox that was signed in through Warmbly Cloud stays in the cloud workspace when you remove it from the instance; disconnecting the instance removes its mirrors of such mailboxes, since they cannot send without the link.
...
| Paid ($15 a month, or $144 a year) | Unlimited | The paid tier first; up to 25 proven free mailboxes fill in only when the paid tier runs thin |

Paid workspaces are always served first. Borrowed partners are limited to mailboxes that have been healthy pool members for at least three days in a workspace in good standing, so nothing unproven ever reaches a paying mailbox.

Upgrade from the link card on your instance: **Settings > Warmbly Cloud** shows an **Unlimited** button once the link is live. It opens Warmbly Cloud, asks whether you want to be billed monthly or yearly, and sends you to checkout for the workspace the instance is linked to. The workspace's regular hosted plan, if it has one, also counts as paid, so a workspace on Starter or above needs nothing extra.

The pool plan is not listed next to Starter, Grow and Business, because it is not an alternative to them: it buys pool access for an instance you run yourself. It has no page of its own on the pricing site and no card in the plans grid, and the button on your instance is the way in.
