...
A mailbox's drawer shows, next to its warmup reputation, how many distinct partners it reached over the last seven days, on how many domains, in how many workspaces. One workspace over a whole week means the mailbox is warming inside your own account, which is worth knowing: it happens when a pool has no outside partner available for it, and on a self-hosted instance that is not linked to Warmbly Cloud it is the only thing that can happen.
