...
1. Verify your account email in [CleanMyList](https://www.cleanmylist.io/), then create a workspace key under **App → API keys**.
...
If both verification services are connected, the most recently created connected or degraded connection takes precedence. Disconnect it to use the other service. See the [CleanMyList API documentation](https://www.cleanmylist.io/developers) and [address verification](/guides/deliverability/#address-verification).
