...
Whichever mode you pick, it decides which mailbox **starts** a lead. Every mailbox still stays inside its own daily cap and minimum gap, so a campaign's volume is the sum of its mailboxes' budgets as always. Rotation only chooses among mailboxes whose minimum gap has passed, and warmup mail counts toward that gap. A mailbox that just sent warmup sits out that turn and another one starts the lead. The campaign only waits when every mailbox is inside its gap.
...
Only an email actually handed to a sending worker counts against a mailbox's daily budget and its minimum gap. The scheduler's own wake-ups, a step it deferred because its slot was not due yet, and action or wait steps never do, so a campaign started outside its sending window still has its whole budget when the window opens. When every mailbox on a campaign has used its budget for the day, the campaign stays active and waits for the next day; the activity log notes it once per day, and it notes the send that used the last of the budget as well, so a campaign that sends one email in the morning and then goes quiet says so in its feed rather than looking stalled. It also waits, rather than pausing, when every mailbox is outside its own hours, resting, or held back by warmup health. A pool that is part spent and part asleep says so too, naming how many of each, because that campaign resumes when the sleeping mailbox reopens rather than tomorrow.

Each of those lines carries a breakdown per mailbox: the cap this campaign gives it today, which limit set that cap (the mailbox's own daily cap, the campaign daily limit, the campaign ramp, the [easing out of warmup](#easing-out-of-warmup) ceiling, or the workspace's [risk band](/guides/deliverability/#workspace-sending-posture)), how much it has sent, the gate that closed it if one did, and its warmup health band when that is dampening it. When a pool's day is shorter than you expected, this is where to look first: a mailbox that is still warming sends `5` cold emails a day at most, whatever its cap says, and three such mailboxes make a `15`-a-day campaign.

### Today's sending plan

The campaign overview opens with a one-line **Today's sending plan**: how many emails the campaign sends today, worked out on the spot through the same gates a real send goes through, and the one limit that decides it. **Why** opens the working: a list of every limit that took the number below what your settings add up to, the leads, and each mailbox's day. Nothing here is stored; the card re-reads itself once a minute while the page is open and on every change to the campaign or its mailboxes.

The number is the sends already out plus what is still expected, and the list under it adds up: the mailbox caps added together, minus each limit, minus what has been sent, is what is still to go. A limit only appears when it removed something. In the order the scheduler applies them:

| Limit | What it means |
| --- | --- |
| Campaign daily limit | The campaign caps each mailbox below the mailbox's own cap |
| Campaign ramp-up | Daily ramp-up is still climbing toward its ceiling |
| Easing out of warmup | A mailbox that warmed starts cold at `5` to `20` a day and adds `5` each clean day (see [easing out of warmup](#easing-out-of-warmup)) |
| Workspace sending posture | A restricted workspace sends a fraction of every cap (see [workspace sending posture](/guides/deliverability/#workspace-sending-posture)) |
| Domain authentication failing, Resting or in reserve, Held by warmup health | A mailbox that sends nothing cold today, and why |
| Used by other campaigns | A mailbox's daily cap is shared by every campaign it is on; these sends went to another campaign today |
| Slowed by warmup health | A mailbox on watch or throttled has its sends spaced wider |
| Outside the mailbox's hours, Sending behaviour plan | A mailbox in another timezone or with its own [workday](/guides/sending-behavior/) is closed for the rest of today, or its rolled plan gives it fewer sends than its cap |
| Minimum gap between sends | With the sending time left today, a mailbox's gap between two sends does not fit any more. Warmup mail sits on the same clock |
| Outside the sending window | The schedule has no sending time left today |
| Plan's daily allowance | The workspace's plan caps campaign emails a day across every campaign (a free trial allows `20`) |
...
A new campaign follows the workspace timezone, set under **Settings > Profile > Timezones** (which also lists every campaign's and mailbox's timezone, each changeable there in one place), until you give it one of its own on its Schedule tab; while the workspace has no timezone, the wizard starts from your browser's instead. A campaign created through the API without a timezone follows the workspace too and reports the zone in use as `effective_timezone`. Check the zone when a campaign is meant for another region: an `8am` to `6pm` window is read in that zone, so a campaign started in the morning in New York on a London schedule has already used most of its day.
...
The detail view streams sends, opens, clicks, replies, bounces, and skips as they happen, for every teammate at once. The **Sending** card appears only while an email is actually being handed to a worker, and always names the contact; between sends the panel shows the campaign's progress and the log, not a send in flight. A **Needs attention** panel above it surfaces failures such as a step that could not be scheduled, a send the worker could not complete, or a mailbox whose sending worker is offline; these retry automatically and stay visible so nothing fails silently.
...
It runs on the AI provider configured for the deployment (see [AI credits](/guides/ai-credits/) for the cost, and [Configuration](/development/configuration/) for self-host setup). It needs the **Use AI** permission. A deployment with neither an AI provider nor TypeSafe configured says so in place of the button rather than offering something that cannot run.

When the deployment has TypeSafe configured (`TYPESAFE_API_KEY`), the same click also returns a **Reads as** block: how the copy lands with the person receiving it, on a personal-note-to-bulk-mail scale; whether it is written for this reader or could be sent to anyone; whether it makes one clear ask, several, or none; and a warning when it makes a claim a spam filter would object to. These are calibrated numbers from a reader model, not prose, and the subject and body are sent to TypeSafe to get them. On a deployment with TypeSafe and no AI provider, the button still works: it returns the rules score, the Reads as block and a one-line reading in place of the quoted findings, and spends no credits.
