...
1. Turn on 2-Step Verification on the Google account, under [Google Account, Security](https://myaccount.google.com/security). Google only offers app passwords once it is on.
...
3. Turn IMAP on in Gmail: **Settings**, **See all settings**, **Forwarding and POP/IMAP**, **Enable IMAP**. On Google Workspace an administrator may have to allow IMAP and app passwords for the organization first.
