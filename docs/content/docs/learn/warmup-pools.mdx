...
Separation has to stay an exchange, though. A tier that only sends into the other and never receives builds nothing: reputation comes from mail arriving, being read and being answered, not from mail leaving. So the one crossing Warmbly allows is mirrored: a thin premium tier borrows proven free mailboxes as recipients, and those same mailboxes write back to the paying mailboxes that wrote to them. Every draw then favours whichever inbox is furthest behind on what it sent, and caps what any inbox receives in a day, so neither tier is starved and neither is flooded.
