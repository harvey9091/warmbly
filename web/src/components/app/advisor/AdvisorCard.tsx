...
export default function AdvisorCard({ finding, onFix, compact = false, defaultOpen = false }: Props) {
    ...
    return (
        <div
            className={
                compact
                    ? `group transition ${applied ? "bg-emerald-500/[0.07]" : "hover:bg-slate-50/60"}`
                    : `group rounded-md border bg-white/70 backdrop-blur-sm transition ${
                          applied ? "border-emerald-500/25 bg-emerald-500/[0.07]" : "border-slate-200/80 hover:border-slate-300"
                      }`
            }
        >
            <div className={`flex gap-2 px-2.5 ${compact ? "items-center min-h-9 py-1" : "items-start py-2"}`}>
                <span
                    className={`${compact ? "" : "mt-[7px]"} h-1.5 w-1.5 shrink-0 rounded-full ${
                ...
                >
                    <div className="flex items-center gap-1.5 min-w-0">
                        <span className={`truncate ${compact ? "text-[12px]" : "text-[12.5px]"} font-medium text-slate-900`}>{finding.title}</span>
                        {compact && !open ? (
                            <span className="hidden md:inline truncate text-[11.5px] text-slate-500 min-w-0">{finding.detail}</span>
                        ) : null}
            ...
            <AnimatePresence initial={false}>
                {open ? (
                    ...
                    >
                        <div className={`border-t border-slate-200/60 px-2.5 ${compact ? "space-y-1.5 py-2 pl-6" : "space-y-2.5 py-2.5"}`}>
                            <p className={`${compact ? "text-[12px]" : "text-[12.5px]"} leading-relaxed text-slate-600`}>{finding.detail}</p>

                            {compact ? (
                                <p className="text-[12px] leading-relaxed text-slate-700">
                                    <span className="font-medium text-slate-900">What to do: </span>
                                    {finding.remedy}
                                </p>
                            ) : (
