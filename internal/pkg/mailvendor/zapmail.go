...
func newZapmail(vals map[string]string, o options) (*zapmail, error) {
	key := vals[FieldAPIKey]
	var providers []string
	switch sp := strings.ToUpper(vals[FieldServiceProvider]); sp {
	case "":
		providers = []string{"GOOGLE", "MICROSOFT"}
	case "GOOGLE", "MICROSOFT":
		providers = []string{sp}
	default:
		return nil, fmt.Errorf("%w: %s: Mailbox type must be GOOGLE or MICROSOFT", ErrInvalidConfig, VendorZapmail)
	}
	auth := func(h http.Header) { h.Set("x-auth-zapmail", key) }
	return &zapmail{
		t:         newTransport(VendorZapmail, "https://api.zapmail.ai/api", o, auth, zapmailPerSecond),
		workspace: vals[FieldWorkspaceID],
		providers: providers,
	}, nil
}
...
func (c *zapmail) header(provider string) http.Header {
	h := http.Header{}
	if c.workspace != "" {
		h.Set("x-workspace-key", c.workspace)
	}
	if provider != "" {
...
func (c *zapmail) Verify(ctx context.Context) error {
	...
	if err := c.t.do(ctx, call{method: http.MethodGet, path: "/v2/users", header: c.header("")}, &res); err != nil {
		return err
	}
	return res.err()
}
...
func (c *zapmail) List(ctx context.Context) ([]Mailbox, error) {
	...
	for _, provider := range c.providers {
		for page := 1; page <= maxPages; page++ {
			...
			if err := c.t.do(ctx, call{method: http.MethodGet, path: "/v2/mailboxes/list", query: q, header: c.header(provider)}, &res); err != nil {
				return nil, err
			}
			...
			for _, d := range res.Data.Domains {
				for _, m := range d.Mailboxes {
					...
					seen[m.ID] = true
					c.cache.put(m.ID, m.credentials())
...
// Credentials comes from List's cache, else from the mailbox detail endpoint.
func (c *zapmail) Credentials(ctx context.Context, m Mailbox) (Credentials, error) {
	...
	if err := c.t.do(ctx, call{method: http.MethodGet, path: "/v2/mailboxes", query: q, header: c.header(provider)}, &res); err != nil {
...
// Domains pages through GET /v2/domains once per service provider.
func (c *zapmail) Domains(ctx context.Context) ([]Domain, error) {
	...
	for _, provider := range c.providers {
		for page := 1; page <= maxPages; page++ {
			...
			q := url.Values{"page": {strconv.Itoa(page)}, "limit": {strconv.Itoa(zapmailPageSize)}}
			if err := c.t.do(ctx, call{method: http.MethodGet, path: "/v2/domains", query: q, header: c.header(provider)}, &res); err != nil {
				return nil, err
			}
			if err := res.err(); err != nil {
				return nil, err
...
func (c *zapmail) SetForwarding(ctx context.Context, d Domain, target string) error {
	...
	if d.ID == "" {
		return invalid(VendorZapmail, "domain id is required")
	}
	h := c.header(c.domains.get(d.ID))
	var cl call
	if target == "" {
		// This endpoint's page names the workspace header x-workspace-id, so both are sent.
		if c.workspace != "" {
			h.Set("x-workspace-id", c.workspace)
		}
		cl = call{method: http.MethodPost, path: "/v2/domains/remove-forwarding", body: map[string]any{"domainId": d.ID}, header: h}
	} else {
		// contains and tagIds are filters that widen the target set, so they are left out: only this domain changes.
		cl = call{method: http.MethodPost, path: "/v2/domains/forwarding", body: map[string]any{"domainIds": []string{d.ID}, "forwardTo": target}, header: h}
	}
	var res zapmailEnvelope
...
func (c *zapmail) UpsertDNSRecord(ctx context.Context, d Domain, r DNSRecord) error {
	...
	if err := c.t.do(ctx, call{method: http.MethodGet, path: "/v2/dns/", query: url.Values{"id": {d.ID}}, header: h}, &list); err != nil {
		return err
	}
	...
	for _, idx := range p.remove {
		q := url.Values{"id": {existing[idx].ID}, "assignedDomainId": {d.ID}}
		res = zapmailEnvelope{}
